...
class NetworkManager {
    ...
    fun addCustomIp(config: AppConfig): Boolean {
        ...
        val commands = listOf(
            ...
            "ndc interface setcfg $iface $ip $prefix up",
            "toybox ip addr add $ip/$prefix dev $iface",
            "ifconfig ${iface}:0 $ip netmask $mask up",
            "toybox ifconfig ${iface}:0 $ip netmask $mask up",
            "busybox ip addr add $ip/$prefix dev $iface",
            "busybox ifconfig ${iface}:0 $ip netmask $mask up"
        )
    ...
    /**
     * Check if the custom IP is currently assigned. Uses QUIET calls.
     */
    fun hasCustomIp(config: AppConfig): Boolean {
        ...
        run {
            ...
            if (r.success && r.stdout.contains(ip)) return true
            val r2 = ShellExecutor.runRootQuiet("ifconfig ${iface}:0")
            if (r2.success && r2.stdout.contains(ip)) return true
        }
        run {
            val r = ShellExecutor.runRootQuiet("toybox ip addr show $iface")
            if (r.success && r.stdout.contains(ip)) return true
        }
        run {
            val r = ShellExecutor.runRootQuiet("busybox ip addr show $iface")
            if (r.success && r.stdout.contains(ip)) return true
        }
